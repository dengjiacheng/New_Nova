...
NovaServer 需要一组后台任务保证设备状态、执行队列与归档流程的可靠性。这些任务可以通过 FastAPI 的 `BackgroundTasks`、`asyncio` 循环或外部任务队列（如 Celery）实现，本文以内置异步调度为例。
...
- 队列结构：Redis List 或内存队列 `Queue[ExecutionTask]`。
